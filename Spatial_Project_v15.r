...
# Structure follows professor's Boston.r template
# =============================================================================
...
data <- st_read("/Users/manuelmariafrois/Desktop/Spatial Econometrixs/Projeto Espacial/Politics shp/politics.shp")
...
quadrant[demean_CHC > 0 & demean_lmoran > 0] <- 4   
quadrant[demean_CHC < 0 & demean_lmoran < 0] <- 1   
quadrant[demean_CHC < 0 & demean_lmoran > 0] <- 3   
quadrant[demean_CHC > 0 & demean_lmoran < 0] <- 2   
quadrant[lmoran[, 5] > sig]                  <- 0   
...
## Generate spatially lagged X variables
for (v in names_x) {
...
safe_anselin(bptest_slm_wx, lm_tests_wx$RSlag, "SDM (SLM Durbin)")
...
pca_vars <- c("Ghost.Town", "Utopia", "Chega_aign", "Tik.T_.Gen")
pca_data <- st_drop_geometry(data[, pca_vars])
...
set.seed(123)
kmeans_result <- kmeans(pca_scaled, centers = 4, nstart = 25)

data$cluster <- as.factor(kmeans_result$cluster)

plot(poly, 
     col = rainbow(4)[as.integer(data$cluster)],
     main = "K-means Clusters (k=4)")
legend("right",
       legend = paste("Cluster", 1:4),
       fill = rainbow(4),
       bty = "n", cex = 0.8)
cluster_means <- aggregate(pca_data, by = list(Cluster = data$cluster), FUN = mean)
print(cluster_means)

spec <- as.formula("CHC ~ Utopia + Ghost.Town + Tik.T_.Gen + Chega_aign + cluster")

bandwith <- gwr.sel( formula = spec, 
                     coords = coords, 
                     longlat = FALSE, 
                     gweight = gwr.Gauss, 
                     adapt = TRUE, 
...
"Computes the optimal bandwidth, such that it minimizes the mean squared error of the prediction errors.
Basically, this allows to compute the diagonal elements of matrix G. 
Since this function is related with matrix G, we need to provide the coordinates of our spatial system."
...
gwr_df <- as(gwr$SDF, "data.frame")

################################################################################
#                                   Utopia                                     #
################################################################################
Utopia_coef <- gwr_df$Utopia
Utopia_se <- gwr_df$Utopia_se_EDF
t_Utopia <- Utopia_coef / Utopia_se
p_Utopia <- 2 * (1 - pnorm(abs(t_Utopia)))
sig_Utopia <- Utopia_coef
sig_Utopia[p_Utopia > 0.05] <- NA
"Selects only the coefficients significant at the 5% level"
...
colors <- rainbow(6)
par(mar = c(0.1, 0.1, 4, 4))
plot(poly, col=colors[findInterval(sig_Utopia, brks, all.inside= TRUE)], 
     main="GWR estimated effects w.r.t. Utopia", cex.main = 0.8, cex.axis = 1.2)
legend("right", 
       legend = c("high Negative","mod Negative","low Negative","low Positive","mod Positive","high Positive"),
       fill=colors, bty = "n", cex = 0.8)


png("hist_utopia.png", width = 800, height = 600)
par(mar = c(5, 4, 4, 2))
hist(sig_Utopia,
     xlab = "Local GWR Coefficient",
     ylab = "Number of Municipalities",
     main = "Distribution of Local Utopia Coefficients")
dev.off()
################################################################################
#                               TikTok Gen                                     #
################################################################################
TikTok_coef <- gwr_df$Tik.T_.Gen
TikTok_se <- gwr_df$Tik.T_.Gen_se_EDF
t_TikTok <- TikTok_coef / TikTok_se
p_TikTok <- 2 * (1 - pnorm(abs(t_TikTok)))
sig_TikTok <- TikTok_coef
sig_TikTok[p_TikTok > 0.05] <- NA
"Selects only the coefficients significant at the 5% level"

describe(sig_TikTok)
boxplot(sig_TikTok)
hist(sig_TikTok)
"Describe statistics for the significant coefficients" 

brks <- c(-3*sd(TikTok_coef),
          -2*sd(TikTok_coef),
          -1*sd(TikTok_coef),
          0,
          1*sd(TikTok_coef),
...
colors <- rainbow(6)
par(mar = c(0.1, 0.1, 4, 4))
plot(poly, col=colors[findInterval(sig_TikTok, brks, all.inside= TRUE)], 
     main="GWR estimated effects w.r.t. TikTok Generation", cex.main = 0.8, cex.axis = 1.2)
legend("right", 
       legend = c("high Negative","mod Negative","low Negative","low Positive","mod Positive","high Positive"),
       fill=colors, bty = "n", cex = 0.8)
png("hist_tiktok.png", width = 800, height = 600)
par(mar = c(5, 4, 4, 2))
hist(sig_TikTok,
     xlab = "Local GWR Coefficient",
     ylab = "Number of Municipalities",
     main = "Distribution of Local TikTok Gen Coefficients")
dev.off()
################################################################################
#                               GhostTown                                      #
################################################################################
GhostTown_coef <- gwr_df$Ghost.Town
GhostTown_se <- gwr_df$Ghost.Town_se_EDF
t_GhostTown <- GhostTown_coef / GhostTown_se
p_GhostTown <- 2 * (1 - pnorm(abs(t_GhostTown)))
sig_GhostTown <- GhostTown_coef
sig_GhostTown[p_GhostTown > 0.05] <- NA
"Selects only the coefficients significant at the 5% level"

describe(sig_GhostTown)
boxplot(sig_GhostTown)
hist(sig_GhostTown)
"Describe statistics for the significant coefficients" 

brks <- c(-3*sd(GhostTown_coef),
          -2*sd(GhostTown_coef),
          -1*sd(GhostTown_coef),
          0,
          1*sd(GhostTown_coef),
...
colors <- rainbow(6)
par(mar = c(0.1, 0.1, 4, 4))
plot(poly, col=colors[findInterval(sig_GhostTown, brks, all.inside= TRUE)], 
     main="GWR estimated effects w.r.t. GhostTown", cex.main = 0.8, cex.axis = 1.2)
legend("right", 
       legend = c("high Negative","mod Negative","low Negative","low Positive","mod Positive","high Positive"),
       fill=colors, bty = "n", cex = 0.8)


png("hist_GhostTown.png", width = 800, height = 600)
par(mar = c(5, 4, 4, 2))
hist(sig_GhostTown,
     xlab = "Local GWR Coefficient",
     ylab = "Number of Municipalities",
     main = "Distribution of Local Ghost Town Coefficients")
dev.off()

################################################################################
#                            Chega Campaign                                    #
################################################################################ 
Chega_Campaign_coef <- gwr_df$Chega_aign
Chega_Campaign_se <- gwr_df$Chega_aign_se_EDF
t_Chega_Campaign <- Chega_Campaign_coef / Chega_Campaign_se
p_Chega_Campaign <- 2 * (1 - pnorm(abs(t_Chega_Campaign)))
sig_Chega_Campaign <- Chega_Campaign_coef
sig_Chega_Campaign[p_Chega_Campaign > 0.05] <- NA
"Selects only the coefficients significant at the 5% level"

describe(sig_Chega_Campaign)
boxplot(sig_Chega_Campaign)
hist(sig_Chega_Campaign)
"Describe statistics for the significant coefficients" 

brks <- c(-3*sd(Chega_Campaign_coef),
          -2*sd(Chega_Campaign_coef),
          -1*sd(Chega_Campaign_coef),
          0,
          1*sd(Chega_Campaign_coef),
          2*sd(Chega_Campaign_coef),
          3*sd(Chega_Campaign_coef))
colors <- rainbow(6)
par(mar = c(0.1, 0.1, 4, 4))
plot(poly, col=colors[findInterval(sig_Chega_Campaign, brks, all.inside= TRUE)], 
     main="GWR estimated effects w.r.t. Chega Campaign", cex.main = 0.8, cex.axis = 1.2)
legend("right", 
       legend = c("high Negative","mod Negative","low Negative","low Positive","mod Positive","high Positive"),
       fill=colors, bty = "n", cex = 0.8)

png("hist_Chega Campaign.png", width = 800, height = 600)
par(mar = c(5, 4, 4, 2))
hist(sig_Chega_Campaign,
     xlab = "Local GWR Coefficient",
     ylab = "Number of Municipalities",
     main = "Distribution of Local Chega Campaign Coefficients")
dev.off()


##### Moran´s I #####

# Compute GWR pseudo-residuals
pseudo_resid <- as.numeric(gwr$SDF$gwr.e)

# Global Moran's I on pseudo-residuals
moran_gwr <- moran.test(pseudo_resid, listw = wmat, alternative = "two.sided")
print(moran_gwr)

# Local Moran's I on pseudo-residuals
lmoran_gwr <- localmoran(pseudo_resid, listw = wmat, alternative = "two.sided")
hist(lmoran_gwr[,"Ii"], main = "Local Moran's I - GWR Pseudo-residuals")

# Map local Moran's I
sig <- 0.05
quadrant <- rep(0, nrow(data))
demean_resid <- pseudo_resid - mean(pseudo_resid)
demean_lmoran <- lmoran_gwr[,1] - mean(lmoran_gwr[,1])

quadrant[demean_resid > 0 & demean_lmoran > 0] <- 4  
quadrant[demean_resid < 0 & demean_lmoran < 0] <- 1  
quadrant[demean_resid < 0 & demean_lmoran > 0] <- 3  
quadrant[demean_resid > 0 & demean_lmoran < 0] <- 2  
quadrant[lmoran_gwr[,5] > sig] <- 0                 
...
legend("right",
       legend = c("insignificant","low-low","low-high","high-low","high-high"),
       fill=colors1, bty="n", cex=0.8)
mtext("Local Moran's I - GWR Pseudo-residuals", side=3, outer=TRUE, line=1, cex=0.8, font=2)
